...
const int p0 = 1, temp = 10, cooling = 10, steps = 1000000;
// Efforts factors
...
int main()
{
    ...
    getStats();
    wchar_t *randomLayout;
    wchar_t emptyLayout[ALPHABET_LENGTH + 1] = {L'0'};
    randomLayout = generateRandomLayout(emptyLayout);
    OptimalLayout finalLayout = findOptimalLayout(randomLayout);
    FILE* fp = fopen("optimalLayout.txt", "w");
    if(fp == NULL) wprintf(L"Could not create or open optimalLayout.txt\n");
    else
    {
        fwprintf(fp, L"%ls: %.2f", finalLayout.bestLayout, finalLayout.bestEffort);
    }
